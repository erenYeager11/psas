  ...
  return (
    <footer className="bg-primary-700 text-primary-200">
      <div className="container-custom section-padding pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-primary-300 text-xs">Est. {schoolInfo.established}</p>
              </div>
            </div>
            <p className="text-sm text-primary-300 max-w-md leading-relaxed mb-4">
              {schoolInfo.name} has been a pillar of quality education, committed to nurturing
              young minds and shaping future leaders with integrity and excellence.
            </p>
            <div className="flex gap-3">
              {Object.entries(schoolInfo.socialLinks).map(([platform, url]) => (
                <a
                  key={platform}
                  href={url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-primary-600 hover:bg-accent-400 flex items-center justify-center text-white transition-colors duration-200"
                  aria-label={platform}
                >
          ...
          {footerLinks.map((section) => (
            <div key={section.title}>
              <h4 className="text-white font-heading font-semibold mb-4">{section.title}</h4>
        ...
        {/* Divider + Bottom */}
        <div className="border-t border-primary-600 mt-12 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-primary-400">
            © {new Date().getFullYear()} {schoolInfo.name}. All rights reserved.
          </p>
          <div className="flex gap-6 text-xs text-primary-400">
            <span className="flex items-center gap-1">📍 {schoolInfo.address.split(',').slice(-2).join(',')}</span>
            <span className="flex items-center gap-1">📞 {schoolInfo.phone}</span>
          </div>
        </div>
      </div>
    </footer>
